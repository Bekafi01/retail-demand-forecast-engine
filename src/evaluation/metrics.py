...
class WRMSSEEvaluator:
    ...
    def score(
        ...
        overall_mae = compute_mae(merged[self.target_col].values, merged[pred_col].values)
        overall_rmse = compute_rmse(merged[self.target_col].values, merged[pred_col].values)
...
def compute_wrmsse(
    ...
    """Convenience helper function to compute overall WRMSSE."""
    ...
    res = evaluator.score(val_df, pred_df)
    return float(res["wrmsse"])
